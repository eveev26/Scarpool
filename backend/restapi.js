...
async function findOneListingById(client, id) {
    ...
    const result = await client.db("scarpool").collection("drivers").findOne({ _id: o_id });
...
async function createListing(client, newListing){
    const result = await client.db("scarpool").collection("drivers").insertOne(newListing);
    console.log(`New listing created with the following id: ${result.insertedId}`);
    return client.db("scarpool").collection("drivers").find({}).toArray(function(err, result) {
        if (err) throw err;
        console.log(result);
        db.close();
    });;
...
async function updateListingById(client, id, updatedListing) {
    var o_id = new ObjectId(id);
    const result = await client.db("scarpool").collection("drivers").updateOne({ _id: o_id }, { $set: updatedListing });
...
async function deleteListingById(client, id) {
    var o_id = new ObjectId(id);
    const result = await client.db("scarpool").collection("drivers").deleteOne({  _id: o_id });
...
app.post("/", async (req, res) => {
    await client.connect();
    res.json(await createListing(client, {
                name: req.body.name,
...
app.patch("/add/:id", async (req, res) => {
    ...
    if (result.occupied_seats < result.available_seats) {
        res.json(await updateListingById(client, req.params.id, {"occupied_seats": result.occupied_seats + 1}))
